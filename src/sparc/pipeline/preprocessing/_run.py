...
def run(
    ...
    for i in range(n):
        ...
        stack_pha_nii_path = (
            os.path.join(output_dir, f"{stack_file_prefix}_pha.nii.gz") if pha_exist else None
        )
        stack_meta_nii_path = (
            os.path.join(output_dir, f"{stack_file_prefix}.json") if debug else None
        )
...
def run_with_gui(
    ...
    return (stack_mag_nii_paths, stack_pha_nii_paths, stack_infos, gui_elapsed)
