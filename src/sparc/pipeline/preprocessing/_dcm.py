...
def sort_dcm_series(self, input_dcm_dir):
    ...
    for dcm_path in dcm_paths:
        ...
        dcm_type = None
        if (
            self.img_type_dcm_tag is not None
        ...
        dcm_id = None
        if self.stack_id_dcm_tag is not None and hasattr(dcm, self.stack_id_dcm_tag):
            raw_id = dcm[self.stack_id_dcm_tag].value
            if raw_id and dcm_type == "mag":
                dcm_id = f"s{raw_id}"
            if raw_id and dcm_type == "pha":
                dcm_id = (
                    f"s{raw_id-1}"  # Only works if mag/pha are split into two consecutive series
                )
    ...
    for stack_id in sorted(stack_ids_seen):
        ...
        if not os.path.isdir(stack_mag_dcm_dir) or not os.listdir(stack_mag_dcm_dir):
            logging.warning(f"Stack {stack_id}: no magnitude DICOM files found; skipping.")
            continue
...
def read_rr_intervals_from_dcm_series(
    ...
    for dcm_path in dcm_paths:
        dcm = pydicom.dcmread(dcm_path)
        if self.rr_interval_dcm_tag is not None and hasattr(dcm, self.rr_interval_dcm_tag):
...
def read_acquisition_matrix_from_dcm(
    ...
    if self.acq_mat_dcm_tag is not None and hasattr(dcm, self.acq_mat_dcm_tag):
...
def read_slice_thickness_from_dcm(
    ...
    if self.slice_thickness_dcm_tag is not None and hasattr(dcm, self.slice_thickness_dcm_tag):
