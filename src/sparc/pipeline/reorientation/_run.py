...
def run(
    ...
    ctr_heart_mask_path = (
        heart_mask_path.replace(".nii.gz", "_ctr.nii.gz") if heart_mask_path is not None else None
    )
    ctr_aff_path = (
        os.path.join(output_dir, "heart_ctr_aff.txt") if heart_mask_path is not None else None
    )
    ...
    if self.reo_stacks:
        reo_stack_nii_paths = [p.replace(".nii.gz", "_reo.nii.gz") for p in stack_nii_paths]
    ...
    return (reo_cine_nii_path, ctr_cine_nii_path, ctr_heart_mask_path, aff_path, qc_report_path)
...
def run_with_gui(
    ...
    (reo_cine_nii_path, ctr_cine_nii_path, ctr_heart_mask_path, aff_path, reo_qc_report_path) = (
        self.run(
    ...
    if mode in ("manual", "semi_auto") or (mode == "monitored_auto" and not reo_valid):
    ...
    return (reo_cine_nii_path, reo_qc_report_path, gui_elapsed)
