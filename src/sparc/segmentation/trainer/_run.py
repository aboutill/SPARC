...
def run(
    ...
    # Get absolute path
    input_dirs = [os.path.abspath(in_dir) for in_dir in input_dirs]
    ...
    # Setup logging
    log_path = os.path.join(output_dir, "sparc_segmentation_train.log") if log else None
    self.setup_logging(log_path=log_path, verbose=verbose)
    logging.info(f"Device: {self.device}")
    self.print_model_info()

    # Initialize multi-domain datalist
    datalists = [init_datalist(input_dir=input_dir, **self.data_cfg) for input_dir in input_dirs]

    report_path = os.path.join(output_dir, "sparc_segmentation_train.json")
    ...
    models_paths = self.get_models_paths(models_dir, folds=folds)
    ...
    for fold in range(folds):
        ...
        train_transforms = init_train_transforms(**self.transforms_cfg)
        ...
        # Initialize val transforms on original image spacing
        val_org_transforms = init_val_org_transforms(
            pixdim=self.transforms_cfg["pixdim"],
        )

        # Initialize val dataset on original image spacing
